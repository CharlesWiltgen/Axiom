...
Complete reference for `xcui`, the Axiom-bundled CLI that makes iOS-simulator UI and accessibility testing scriptable for coding harnesses. It owns the test-harness semantics AXe and `simctl` lack — waiting on conditions, asserting on the accessibility tree, toggling accessibility settings, handling system permission dialogs, and computing VoiceOver announcements — and is also the front door for input: `xcui tap`/`type`/`swipe` forward to AXe, with a physical touch supplied for `tap`, so input lands without you managing AXe's environment or flags yourself. Every subcommand emits a single compact JSON object with a `tool`/`version` envelope (token-lean for LLM consumers); every subcommand also accepts `--human` for a prose rendering, and exit codes drive pass/fail in scripts.
...
- Understanding how `xcui` auto-resolves the booted simulator, and why it refuses to guess when more than one is booted
- Working out why a tap reported success but nothing happened
...
- **Input verbs** – `tap`, `slider`, `type`, `swipe`, `drag`, `touch`, `gesture`, `button`, `key`, `key-sequence`, `key-combo`, `screenshot` forward to AXe (same flags, same output, same exit code), with `--udid` injected when omitted and exactly one simulator is booted, and `--tap-style physical` added to `tap` when you pass no style; `axe describe-ui` stays bare as the raw tree `xcui` parses
