...
- **Template × category availability matrix** – for each of the 12 templates, which of the 11 CarPlay app categories may use it (with iOS version footnotes for iOS 17+, iOS 26.4+, and iOS 27+ additions)
- **Template depth limits** – per-category maximum push depth (5 for audio, video, communication, EV/parking/public-safety/navigation; 3 for fueling and voice-based conversational; 2-or-3 for driving task and quick-food-ordering depending on iOS version)
...
- **Information template (`CPInformationTemplate`)** – single or two-column static labels with optional footer actions, all except Audio and Video, iOS 14+ (nav bar buttons iOS 16+)
...
- **Search template (`CPSearchTemplate`)** – async-results search bar with handler delivery, Navigation — plus, from iOS 27, Audio/Video, EV charging, Fueling, Parking, and Public Safety (never quick food ordering)
- **Tab bar template (`CPTabBarTemplate`)** – top-level navigation with tab icons (24×24 pt; prefer SF Symbols), all categories
- **Voice control template (`CPVoiceControlTemplate`)** – voice control UI for navigation apps (iOS 12+), driving task and voice-based conversational apps (iOS 26.4+), and the remaining categories on iOS 27+; iOS 27 adds a `backgroundImage` and overlay presentation over another template via `CPInterfaceController.showOverlayTemplate(_:animated:completion:)`
- **Push and pop conventions** – `CPInterfaceController.pushTemplate(_:animated:completion:)` and `popTemplate(animated:completion:)`, the runtime exception you get for exceeding depth, and the runtime exception you get for using an unsupported template in your category

## Documentation Scope

This page documents the `carplay-templates-ref` skill — the template catalog half for CarPlay apps.

- For **app design discipline** (category selection, the 7 Universal Guidelines, per-category rules, entitlement review), see [CarPlay HIG](/skills/integration/carplay-hig). Start there before picking templates.
...
- CarPlay Developer Guide (Jun 2026) — template × category matrix and depth limits (p.14), per-template specs (p.15-26), asset sizes (p.28, p.39), Now Playing template code (p.33)
