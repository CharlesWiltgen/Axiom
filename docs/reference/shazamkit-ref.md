...
- **`SHManagedSession` (iOS 17+)** – initializer overloads (default catalog vs custom catalog), single-shot `result() async`, `Results` `AsyncSequence` for continuous matching, `prepare() async` for pre-recording, `cancel()` to stop, `state` Observable property with `.idle` / `.prerecording` / `.matching` cases, `Sendable` conformance as of iOS 17
