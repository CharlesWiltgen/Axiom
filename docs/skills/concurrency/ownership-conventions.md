...
- Value generics
- Swift 6.4 (iOS 27): `borrow`/`mutate` accessors, `~Copyable` `Equatable`/`Comparable`/`Hashable`, the `[N of T]` InlineArray literal, `Ref`/`MutableRef`/`UniqueBox`/`UniqueArray` ownership containers
