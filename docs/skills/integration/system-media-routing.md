...
Reportedly driven by the EU Digital Markets Act, this capability is **likely region-gated (EU)**. Treat third-party routes as *may or may not be present*: always `#available`-gate and keep your existing AirPlay / in-app cast path as the fallback.
:::
...
- **`playbackControl` vs `dataChannel`** – AVKit's `AVPlaybackUserInterfaceControllable` for standard playback control/observation, or raw `Data` exchange for custom protocols. (Do **not** use `AVInterfaceControllable`: Apple shipped that family in an early 27 beta and then removed it, so code or AI suggestions that still reference it no longer compile.)
- **What `playbackControl` actually gives you** – which members you set (`isPlaying`, `volume`, `state`) and call (`seek(to:tolerance:)`) versus read (`isReady`, `isBuffering`, `segments`, `seekableTimeRanges`), and that it is `@MainActor`-isolated and `Observable`
...
- **Ad-gating the scrubber** – nothing enforces it for you: `requiresLinearPlayback` is a read-only indicator, and `seekableTimeRanges` only *typically* excludes ad segments (with `nil` and `[]` meaning **opposite** things). Covers the three-source gate — `supportedSeekCapabilities`, `seekableTimeRanges`, and subtracting the `requiresLinearPlayback` segments
