...
- **Orientation as a preference** – lock is a request, not a guarantee, and on iPad `requestGeometryUpdate` is refused outright under the windowing model (the error handler is the only signal)
