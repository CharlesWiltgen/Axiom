...
- **textkit-auditor** – Scans for TextKit issues and architectural gaps (TextKit 1 fallback triggers, glyph-counted APIs that break on complex scripts, missing Writing Tools configuration, missing `isWritingToolsActive` guards on programmatic mutations, SwiftUI wrappers dropping TextKit 2 properties, RTL-untested custom layout fragments)

### UI & Design
- **liquid-glass-auditor** – Identifies iOS 26+ Liquid Glass adoption opportunities and completeness gaps (glass effects, semantic toolbar placement, migration from old blur effects, Regular vs Clear variant discipline for media surfaces, glass-on-glass nesting, unstyled pre-26 fallbacks, accessibility re-check after adoption, `Tab(role: .search)`, the `UIDesignRequiresCompatibility` opt-out); scores ADOPTED / PARTIAL / NOT ADOPTED
