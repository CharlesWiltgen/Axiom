...
| Automated resize-readiness scan (scene lifecycle, UIScreen.main, fixed-canvas, Mirroring input, iPhone Duo) | Delegate to the `resize-auditor` subagent or `/axiom-audit` resize |
...
→ Delegate to the `resize-auditor` subagent (or `/axiom-audit` resize)
