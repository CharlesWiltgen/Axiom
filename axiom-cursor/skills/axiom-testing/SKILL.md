...
**Test quality audit** → Delegate to the `testing-auditor` subagent or `/axiom-audit` testing (maps test coverage shape against production code, detects flaky patterns and speed issues, identifies untested critical paths, scores overall test health)

**Flaky test analysis** → Delegate to the `test-failure-analyzer` subagent (scans for patterns causing intermittent failures in Swift Testing: missing confirmation, shared mutable state, missing @MainActor)
