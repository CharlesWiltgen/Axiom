...
description: "Use this agent when the user mentions Codable review, JSON encoding/decoding issues, data serialization audit, or modernizing legacy code."
model: inherit
readonly: true
is_background: true
---
...
**Search**: `\{\\"` (a `{` followed by an escaped quote), `"\{` (a quote followed by a `{`), `\+ "\\"` (a `+` concatenating a quote) — inside string literals containing `{` or `}`
...
assistant: [Delegate to the `codable-auditor` subagent]
</example>
