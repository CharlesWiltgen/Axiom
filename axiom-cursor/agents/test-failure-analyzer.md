...
description: "Use this agent when the user mentions flaky tests, tests that pass locally but fail in CI, race conditions in tests, or needs to diagnose WHY a specific test fails."
model: inherit
readonly: true
is_background: true
---
...
# Run `axiom_xcsym_crash` MCP tool --format=summary on each to get pattern_tag + crashed frames
```
...
assistant: [Delegate to the `test-failure-analyzer` subagent]
</example>
