...
Delegate to the `health-check` subagent to perform a comprehensive project audit.
...
If `diff` is present, compute the file scope yourself before Delegate to the `the` subagent (the agent has no Bash tool):
