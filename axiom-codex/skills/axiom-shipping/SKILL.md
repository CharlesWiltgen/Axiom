...
| Production corpus triage (Sentry, ASC — multiple grouped issues) | See `skills/production-triage.md` + `triage-analyzer` agent or `axiom-analyze-triage` |
...
**Invoke**: `axiom-validate-screenshots`
...
**Reference**: `skills/production-triage.md` (fetch + NormalizedReport schema + flag-never-hide rule). **Agent**: `triage-analyzer` or `axiom-analyze-triage` / `axiom-analyze-triage`.
...
→ `axiom-analyze-triage` (or `axiom-analyze-triage`)
