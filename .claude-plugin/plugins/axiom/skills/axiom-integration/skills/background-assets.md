...
> **27 SDK status**: the `SystemLanguageModel.Adapter` runtime is retroactively deprecated from 26.4 and **obsoleted at 27.0** in the 27 SDK — this pattern compiles only for pre-27 deployment targets, and as of the 27 SDK still names no replacement. The Background Assets delivery side is unaffected.
