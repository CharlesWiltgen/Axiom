...
The intent updates shared data via App Groups in its `perform()` method. See **references/app-intents-ref.md** for full `AppIntent` definition syntax.
...
The intent follows the same `AppIntent` structure with a `@Parameter(title: "Enabled") var enabled: Bool`. See **references/app-intents-ref.md** for full `AppIntent` definition syntax.
...
**Skills**: references/app-intents-ref.md, axiom-concurrency, axiom-swiftui, references/extensions-widgets.md
