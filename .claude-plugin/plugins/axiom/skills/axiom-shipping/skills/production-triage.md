...
```json
{
  "provider": "sentry",
  "issue_id": "APP-3V",
  "issue_url": "https://sentry.io/organizations/acme/issues/APP-3V/",
...
```json
{
  "provider": "sentry",
  "issue_id": "APP-7B",
  "issue_url": "https://sentry.io/organizations/acme/issues/APP-7B/",
...
**Never omit a noise-flagged issue from the report.** Omission is the correctness failure this architecture is designed to prevent — the ExampleApp lesson was that the #1 issue by user count was an idle-runloop suspension false-positive. A tool that silently buried it would commit the same error inverted.
