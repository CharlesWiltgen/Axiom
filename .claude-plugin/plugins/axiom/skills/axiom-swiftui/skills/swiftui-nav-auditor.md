<!-- GENERATED from agents/swiftui-nav-auditor.md by scripts/build-inlined-auditors.ts — do not edit. -->

# SwiftUI Navigation Auditor

**Claude Code** — launch the `swiftui-nav-auditor` agent, or run `/axiom:audit swiftui-nav`. It runs this procedure in an isolated context with its own model tier.
