<!-- GENERATED from agents/security-privacy-scanner.md by scripts/build-inlined-auditors.ts — do not edit. -->

# Security & Privacy Scanner

**Claude Code** — launch the `security-privacy-scanner` agent, or run `/axiom:audit security`. It runs this procedure in an isolated context with its own model tier.
