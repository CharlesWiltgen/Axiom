...
if [ $? -ne 0 ]; then
  echo "$_axiom_leaks" | grep -E '^  ✗|error\(s\)' | head -20 >&2
  echo >&2 "axiom: leak scan FAILED — private data in shipped content."
...
if [ -n "$_axiom_staged" ]; then
  ...
  deno run --allow-read --allow-run --allow-env scripts/pre-deploy.ts --static 2>&1
  if [ $? -ne 0 ]; then
